...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def activate(self, request, pk=None):
        user = self.get_object(); user.is_active = True; user.updated_by = request.user; user.save(update_fields=["is_active", "updated_by"])
        return Response(self.get_serializer(user).data)

    @action(detail=True, methods=["post"])
    def deactivate(self, request, pk=None):
        user = self.get_object(); user.is_active = False; user.updated_by = request.user; user.save(update_fields=["is_active", "updated_by"])
        return Response(self.get_serializer(user).data)
    ...
    def my_permissions(self, request):
        ...
        permissions = {
            ...
            "create_events": is_admin or (is_supervisor and bool(configuration and configuration.allow_supervisor_create_events)) or (is_advisor and bool(configuration and configuration.allow_advisor_create_events)),
            "reassign_events": is_admin or (is_supervisor and bool(configuration and configuration.allow_supervisor_reassign_events)),
            "edit_advisor_availability": is_admin or (is_supervisor and bool(configuration and configuration.allow_supervisor_edit_availability)) or (is_advisor and bool(configuration and configuration.allow_advisor_edit_availability)),
