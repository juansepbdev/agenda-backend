...
class ChatbotClientSerializer(serializers.Serializer):
    """Datos del cliente que envía el chatbot."""
...
class ChatbotEventSerializer(serializers.Serializer):
    """Datos de la visita que envía el chatbot."""

    title = serializers.CharField(max_length=255)
    start_at = serializers.DateTimeField()
    duration_minutes = serializers.IntegerField(min_value=1, max_value=480, default=60)
    description = serializers.CharField(required=False, allow_blank=True, default="")
    property_external_id = serializers.CharField(max_length=128, required=False, allow_blank=True, default="")
    property_code = serializers.CharField(max_length=128, required=False, allow_blank=True, default="")
    property_title = serializers.CharField(max_length=255, required=False, allow_blank=True, default="")
    property_address = serializers.CharField(max_length=255, required=False, allow_blank=True, default="")
    property_url = serializers.URLField(required=False, allow_blank=True, default="")


class ChatbotRequestSerializer(serializers.Serializer):
    """Alta de visita desde el chatbot.

    `client` y `event` eran `DictField()` sin esquema: cualquier clave que
    faltara (`phone`, `title`, `start_at`) reventaba con KeyError, es decir un
    500 en el endpoint expuesto a un sistema externo. Ahora se validan.
    """

    idempotency_key = serializers.CharField(max_length=255)
    client = ChatbotClientSerializer()
    event = ChatbotEventSerializer()
    chatbot_conversation_id = serializers.CharField(required=False, allow_blank=True)
    chatbot_message_id = serializers.CharField(required=False, allow_blank=True)

    def validate(self, data):
        event = data["event"]
        data["start_at"] = event["start_at"]
        data["end_at"] = event["start_at"] + timedelta(minutes=event["duration_minutes"])
        return data
...
def _event_response(event):
    return {
        ...
        "client": ({"id": str(event.client_id), "name": str(event.client)} if event.client_id else None),
...
def chatbot_event(request):
    ...
    existing = (
        ...
        .filter(company=company, idempotency_key=data["idempotency_key"])
        .select_related("advisor__user", "client")
        .first()
    )
    if existing:
    ...
    try:
        ...
        # normalize_phone rechaza números imposibles con ValueError.
        raise serializers.ValidationError({"client": {"phone": [str(exc)]}}) from exc
    configuration = get_configuration(company)
    advisor = assign_advisor_automatically(
        company=company, start_at=data["start_at"], end_at=data["end_at"], configuration=configuration
    )
    ...
    event = Event.objects.create(
        ...
        end_at=data["end_at"],
        property_external_id=event_data.get("property_external_id", ""),
        property_code=event_data.get("property_code", ""),
        property_title=event_data.get("property_title", ""),
        property_address=event_data.get("property_address", ""),
        property_url=event_data.get("property_url", ""),
        chatbot_conversation_id=data.get("chatbot_conversation_id", ""),
        chatbot_message_id=data.get("chatbot_message_id", ""),
        idempotency_key=data["idempotency_key"],
        assigned_automatically=True,
        requires_confirmation=configuration.require_advisor_confirmation if configuration else False,
    )
    ...
    return Response(
        {
            ...
            "advisors": [
                {"id": str(advisor.id), "name": advisor.user.get_full_name(), "priority": advisor.assignment_priority}
...
def chatbot_cancel_event(request, event_id):
    ...
    event = (
        Event.objects.select_for_update()
        .select_related("advisor__user", "client")
        .filter(company=company, pk=event_id)
        .first()
    )
    if event is None:
